...
@Composable
private fun IncomesTab(incomes: List<Income>, onEdit: (Income) -> Unit, onDelete: (Income) -> Unit) {
    if (incomes.isEmpty()) {
        EmptyState(icon = Icons.Filled.Add, title = stringResource(R.string.budget_add_income), modifier = Modifier.fillMaxSize())
        return
    }
            ...
            Card(modifier = Modifier.fillMaxWidth().clickableCard { onEdit(income) }) {
                Row(modifier = Modifier.fillMaxWidth().padding(12.dp), horizontalArrangement = Arrangement.SpaceBetween, verticalAlignment = Alignment.CenterVertically) {
                    Column {
                        ...
                        Text(
                            frequencyLabel(income.frequency, income.dayOfMonth, income.dayOfWeek, income.monthOfYear, income.oneTimeDate) + if (!income.isActive) " · inactif" else "",
                    ...
                    Row(verticalAlignment = Alignment.CenterVertically) {
                        ...
                        IconButton(onClick = { onDelete(income) }) { Icon(Icons.Filled.Delete, contentDescription = null) }
                    }
                }
            }
        }
...
@Composable
private fun ExpensesTab(expenses: List<RecurringExpense>, categories: List<com.budgetflow.app.domain.model.Category>, onEdit: (RecurringExpense) -> Unit, onDelete: (RecurringExpense) -> Unit) {
    if (expenses.isEmpty()) {
        EmptyState(icon = Icons.Filled.Add, title = stringResource(R.string.budget_add_expense), modifier = Modifier.fillMaxSize())
        return
    }
    LazyColumn(modifier = Modifier.fillMaxSize().padding(16.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
                ...
                Row(modifier = Modifier.fillMaxWidth().padding(12.dp), horizontalArrangement = Arrangement.SpaceBetween, verticalAlignment = Alignment.CenterVertically) {
                    Row(verticalAlignment = Alignment.CenterVertically, horizontalArrangement = Arrangement.spacedBy(12.dp)) {
                        ...
                        Column {
                            ...
                            Text(
                                frequencyLabel(expense.frequency, expense.dayOfMonth, expense.dayOfWeek, expense.monthOfYear, expense.oneTimeDate) + if (!expense.isActive) " · inactif" else "",
                                style = MaterialTheme.typography.bodySmall,
                                color = MaterialTheme.colorScheme.onSurfaceVariant
                            )
                        }
                    ...
                    Row(verticalAlignment = Alignment.CenterVertically) {
                        ...
                        IconButton(onClick = { onDelete(expense) }) { Icon(Icons.Filled.Delete, contentDescription = null) }
                    }
                }
            }
        }
...
@Composable
private fun EnvelopesTab(envelopes: List<EnvelopeItem>, onEdit: (VariableBudget) -> Unit, onDelete: (VariableBudget) -> Unit) {
    if (envelopes.isEmpty()) {
        EmptyState(icon = Icons.Filled.Add, title = stringResource(R.string.budget_add_envelope), modifier = Modifier.fillMaxSize())
        return
    }
    LazyColumn(modifier = Modifier.fillMaxSize().padding(16.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
            ...
            Card(modifier = Modifier.fillMaxWidth().clickableCard { onEdit(item.budget) }) {
                Column(modifier = Modifier.fillMaxWidth().padding(12.dp)) {
                    Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween, verticalAlignment = Alignment.CenterVertically) {
                        ...
                        IconButton(onClick = { onDelete(item.budget) }) { Icon(Icons.Filled.Delete, contentDescription = null) }
                    }
...
@Composable
private fun GoalsTab(goals: List<SavingsGoal>, onEdit: (SavingsGoal) -> Unit, onDelete: (SavingsGoal) -> Unit) {
    if (goals.isEmpty()) {
        EmptyState(icon = Icons.Filled.Add, title = stringResource(R.string.budget_add_goal), modifier = Modifier.fillMaxSize())
        return
    }
    LazyColumn(modifier = Modifier.fillMaxSize().padding(16.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
            ...
            Card(modifier = Modifier.fillMaxWidth().clickableCard { onEdit(goal) }) {
                Column(modifier = Modifier.fillMaxWidth().padding(12.dp)) {
                    Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween, verticalAlignment = Alignment.CenterVertically) {
                        ...
                        IconButton(onClick = { onDelete(goal) }) { Icon(Icons.Filled.Delete, contentDescription = null) }
                    }
                    LinearProgressIndicator(
